...
void UnderGenSceneSpawnerNode::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_zone_scene_map", "map"), &UnderGenSceneSpawnerNode::set_zone_scene_map);
    ClassDB::bind_method(D_METHOD("get_zone_scene_map"), &UnderGenSceneSpawnerNode::get_zone_scene_map);
    ...
    ADD_PROPERTY(PropertyInfo(Variant::DICTIONARY, "zone_scene_map", PROPERTY_HINT_NONE, "String → PackedScene map"), "set_zone_scene_map", "get_zone_scene_map");
...
void UnderGenSceneSpawnerNode::set_scene_to_spawn(const Ref<PackedScene> &p_scene) { scene_to_spawn = p_scene; }
Ref<PackedScene> UnderGenSceneSpawnerNode::get_scene_to_spawn() const { return scene_to_spawn; }
void UnderGenSceneSpawnerNode::set_zone_scene_map(const Dictionary &p_map) { zone_scene_map = p_map; }
Dictionary UnderGenSceneSpawnerNode::get_zone_scene_map() const { return zone_scene_map; }
...
void UnderGenSceneSpawnerNode::execute_with_parent(const Dictionary &inputs, Dictionary &outputs, Node3D* parent_node) {
    ...
    for (const auto& p : raw) {
        ...
        // Determine which scene to use: check zone_scene_map first, fall back to scene_to_spawn
        Ref<PackedScene> chosen_scene = scene_to_spawn;
        if (!zone_scene_map.is_empty()) {
            String zone_name = p.attributes.get("zone_name", "");
            if (zone_scene_map.has(zone_name)) {
                chosen_scene = zone_scene_map[zone_name];
            }
        }

        if (chosen_scene.is_null()) {
            UtilityFunctions::printerr("UnderGenSceneSpawnerNode: No scene for zone '", p.attributes.get("zone_name", ""), "' and no fallback.");
            continue;
        }

        Node* instance = chosen_scene->instantiate();
